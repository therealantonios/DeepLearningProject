'''
This coded is based on https://github.com/cad0p/maskrcnn-modanet.git
It's intended for academic purposes
'''
...
def get_args():
    ...
    parser.add_argument('--new_annotations_path', type=str, default='./dataset/annotations',
                        help='Path where to save new annotations file')
    parser.add_argument('--old_annotations_path', type=str, default='./dataset/annotations',
                        help='Path where old annotations are saved')
...
def main(args):
    ...
    path = args.new_annotations_path
    ann_path = args.new_annotations_path
    ann_orig_path = args.new_annotations_path
    ...
    print(
        'Now fixing annotations. Just delete the file \'instances_all.json\' and rerun this command, if you want to '
        'restore it to the original one.')
    print(
        'It is also useful to delete the file above if you run into any issues at all. It will be recreated '
        'automatically')

    if not os.path.isfile(ann_path + 'modanet_train.json'):
        print('Fixing from the original annotations:\n' + ann_orig_path + 'modanet_train.json')
        with open(ann_orig_path + '/modanet_train.json') as f:
            instances = json.load(f)
    else:
        print('Using ' + ann_path + 'modanet_train.json' + '\nDelete that file if you want to fix the original')
        with open(ann_path + '/modanet_train.json') as f:
            instances = json.load(f)
    ...
    print(
        str(counter_shapes_corrected_double) + 'shapes that were wrongly readded (duplicates) in previous attempts, '
                                               'rest: ' + str(
            counter_shapes_moved))
    ...
    with open(ann_path + '/fixed_modanet_train.json', 'w') as outfile:
